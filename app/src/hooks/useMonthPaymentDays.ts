...
type UseMonthPaymentDaysResult = {
  dates: Set<string>;
  loading: boolean;
  error: string;
};
...
export function useMonthPaymentDays(
  monthDate: Date,
  refreshKey = 0,
): UseMonthPaymentDaysResult {
  ...
  useEffect(() => {
    ...
    const loadMonth = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const message =
            err instanceof Error ? err.message : 'Unable to load payment days.';
          setError(message);
        }
      } finally {
